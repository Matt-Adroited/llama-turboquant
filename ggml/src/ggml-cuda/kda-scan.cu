...
// HIP compatibility: HIP doesn't have __shfl_down_sync for float
#ifdef __HIP_PLATFORM_AMD__
#define KDA_SHFL_DOWN(var, delta) __shfl_down(var, delta)
#else
#define KDA_SHFL_DOWN(var, delta) __shfl_down_sync(0xffffffff, var, delta)
#endif
    ...
    const int64_t y_off)  // offset to state output in dst (in floats)
{
    ...
    for (int t = 0; t < n_seq_tokens; ++t) {
        ...
        for (int offset = warpSize/2; offset > 0; offset /= 2) {
            q_sq_sum += KDA_SHFL_DOWN(q_sq_sum, offset);
            k_sq_sum += KDA_SHFL_DOWN(k_sq_sum, offset);
        }
